...
def totalDiskSpace = 70000000
def unused = 30000000
def fileSystem = buildFileSystem(input)
def gap = unused - (totalDiskSpace - fileSystem.getRoot().totalSize)
def dirs = findDir(fileSystem.getRoot())
def find = dirs.sort { it.totalSize }
        .find { it.totalSize > gap }

assert 7991939L == find.totalSize
